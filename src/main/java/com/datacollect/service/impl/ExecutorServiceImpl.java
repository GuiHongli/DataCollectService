...
public class ExecutorServiceImpl extends ServiceImpl<ExecutorMapper, Executor> implements ExecutorService {
    ...
    public List<Executor> getExecutorsByRegion(Long regionId, Long countryId, Long provinceId, Long cityId) {
        ...
        applyRegionFilters(queryWrapper, regionId, countryId, provinceId, cityId);

        queryWrapper.orderByAsc("name");
        List<Executor> executors = list(queryWrapper);
        logExecutors(executors);
        return executors;
    }

    private QueryWrapper<Executor> baseOnlineExecutorQuery() {
        QueryWrapper<Executor> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("status", 1);
        return queryWrapper;
    }

    private void applyRegionFilters(QueryWrapper<Executor> queryWrapper, Long regionId, Long countryId, Long provinceId, Long cityId) {
        if (cityId != null) {
            filterByCity(queryWrapper, cityId);
            return;
        }
        if (provinceId != null) {
            filterByProvince(queryWrapper, provinceId);
            return;
        }
        if (countryId != null) {
            filterByCountry(queryWrapper, countryId);
            return;
        }
        if (regionId != null) {
